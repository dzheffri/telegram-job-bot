...
async def send_job(title, link, company):
    ...
    keyboard = [[InlineKeyboardButton("✅ Уже откликнулся", callback_data=f"done|{link}")]]
    try:
        ...
        await asyncio.sleep(2)  # пауза между сообщениями, чтобы не перегружать Telegram
    except Exception as e:
        print(f"❌ Ошибка отправки вакансии: {title} | {e}")
